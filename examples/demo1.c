...
{
	double dataset_in[2000] = {0.0};
	double dataset_out[1000] = {0.0};
	struct ann ann = ann_create(2, 20, 1);
	double net_in[2] = {0.0};
	double net_out = 0.0;
	double calc = 0.0;
	double normalized = 0.0;
	ann.max_epoch = 500000;
	ann.lrate = 0.005;
	ann.target = 3;
	ann.max_norm = 10000;
	ann.debug = 1;
	...
	// training
	ann_teach(&ann, dataset_in, dataset_out, 1000);
	...
	//network run
	printf("\n\n\nTraining Result\n==========\n");
	printf("value1 + value2\t\texpected\tmodeled\t\tdiff\n\n");
	for (i = 0; i < 100; i++)
	{
		net_in[0] = i;
		net_in[1] = 2;
